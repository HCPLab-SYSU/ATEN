...
def minimize_mask(bbox, mask, mini_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        m = m[y1:y2, x1:x2]
        if m.size == 0:
            print("invalid bounding box is ", bbox[i][:4])
...
def unmold_part(part, bbox, image_shape):
    """Converts a part generated by the neural network into a format similar
    to it's original shape.
    part: [PART_SHAPE[0], PART_SHAPE[1], NUM_PART_CLASS] 
            of type float. A small, typically 28x28 part.
    bbox: [y1, x1, y2, x2]. The box to fit the part in.

    Returns a uint8 part[image_height, image_width]
            with the same size as the original image.
    """
    y1, x1, y2, x2 = bbox

    part = skimage.transform.resize(part, (y2 - y1, x2 - x1))
    part = np.argmax(part, axis=2).astype(np.uint8)

    # Put the part in the right location.
    full_part = np.zeros(image_shape[:2], dtype=np.uint8)
...
def generate_pyramid_anchors(scales, ratios, feature_shapes, feature_strides,
    ...
    for i in range(len(scales)):
        ...
        # print("anchor num", len(anchors[-1]))
